...
def dashboard():
    ...
    # --- Compute column totals & hide zero-only rows/columns ---
    column_totals = {d: 0 for d in all_delivery_dates}
    for item in pivot_data:
        for d, qty in item["dates"].items():
            column_totals[d] += qty

    # Remove rows that have all zero quantities
    pivot_data = [
        item for item in pivot_data
        if any(qty != 0 for qty in item["dates"].values()) or item["grand_total"] != 0
    ]

    # Remove columns (dates) with zero totals
    non_zero_dates = [d for d, total in column_totals.items() if total != 0]
    column_totals = {d: column_totals[d] for d in non_zero_dates}

    for item in pivot_data:
        item["dates"] = {d: item["dates"][d] for d in non_zero_dates}

    grand_total_all = sum(column_totals.values())
    ...
    return render_template(
        ...
        all_review_dates=all_review_dates,
        all_delivery_dates=non_zero_dates,
